...
const ReaderPage: React.FC<ReaderPageProps> = ({ currentIndex, onChapterChange, language, isLightTheme, readerFont, readingMode = 'standard', onJumpToSideStory }) => {
    ...
    const flushTextBuffer = () => {
        if (textBuffer.length > 0) {
            ...
            if (/^(零点|Point|零點)(:|：|\(|（)/.test(joinedText)) {
                ...
                className = isLightTheme ? `mb-4 md:mb-8 text-justify indent-6 md:indent-12 text-xs md:text-base leading-relaxed text-emerald-700 font-bold ${getFontClass(readerFont)}` : `mb-4 md:mb-8 text-justify indent-6 md:indent-12 text-xs md:text-base leading-relaxed text-emerald-400 drop-shadow-[0_0_2px_rgba(52,211,153,0.4)] ${getFontClass(readerFont)}`;
                lastSpeakerClass = className;
                foundSpeaker = true;
                            ...
                            <React.Fragment key={chapter.id}>
                                ...
                                {index === 2 && (
                                    <Reveal delay={index * 50 + 25}>
                                        <button
                                            onClick={() => onJumpToSideStory && onJumpToSideStory('VOL_PB')}
                                            className={`
                                                w-full h-40 md:h-56 relative group transition-all duration-300 flex flex-col text-left overflow-hidden border-2
                                                ${isLightTheme 
                                                    ? 'bg-black border-zinc-800 text-white hover:shadow-xl' 
