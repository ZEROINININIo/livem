...
export const parseChapterToVN = (rawText: string): VNNode[] => {
  ...
  lines.forEach((line) => {
    ...
    if (trimmed.startsWith('[[DIVIDER]]')) {
        flushBuffer();
        // Insert a narration node to represent the divider visually in text flow if possible, 
        // or just a placeholder. Using symbols to denote scene change.
        const id = `node-${idCounter++}`;
        nodes.push({ id, type: 'narration', text: '— — —', emotion: 'neutral' });
        return;
    }
